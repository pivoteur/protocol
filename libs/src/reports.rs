...
use super::{
    tables::{c2t, csv2tsv},
    types::{measurable::Measurable, proposals::proposes::Propose, util::Pool},
};
...
fn print_row<T: CsvWriter + CsvHeader>(
    printer: impl Fn(&String) -> (),
    first_time: &mut bool,
    row: &T,
) {
    if *first_time {
...
fn print_tsv_row<T: CsvWriter + CsvHeader>(
    printer: impl Fn(&String) -> (),
    first_time: &mut bool,
    row: &T,
) {
...
impl CsvHeader for Proposal {
    fn header(&self) -> String {
        format!(
            "pool,open_pivots,last_pivot_on_dt,{}",
            self.proposal.header()
        )
    }
}
...
pub fn report_proposes(proposes: &[Proposal], no_closers: &[Pool], min: bool) {
    ...
    compact(
        &format!("Pivot pools {pools}"),
        "No pools without close calls",
        no_closers,
        proposes.first(),
        12,
        min,
